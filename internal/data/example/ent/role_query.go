...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	for name, query := range rq.withNamedMenus {
		if err := rq.loadMenus(ctx, query, nodes,
			func(n *Role) { n.appendNamedMenus(name) },
			func(n *Role, e *Menu) { n.appendNamedMenus(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoleQuery) WithNamedMenus(name string, opts ...func(*MenuQuery)) *RoleQuery {
	query := (&MenuClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedMenus == nil {
		rq.withNamedMenus = make(map[string]*MenuQuery)
	}
	rq.withNamedMenus[name] = query
	return rq
}
