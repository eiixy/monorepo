...
func (r userRoleCountLoader) Records(ctx context.Context, keys dataloader.Keys) (map[dataloader.Key]int, error) {
	...
	rows, err := r.client.QueryContext(ctx, fmt.Sprintf(`SELECT user_id, COUNT(role_id) FROM users LEFT JOIN user_roles ON users.id = user_roles.user_id WHERE user_id IN (%s) GROUP BY user_id`, placeholder), gql.ToAnySlice(keys)...)
	if err != nil {
		return nil, err
	}
